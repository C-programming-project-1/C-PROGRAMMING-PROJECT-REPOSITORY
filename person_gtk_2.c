 ...
 *
 * Changes v3:
 *  - ID fully invisible to user (auto-generated, never displayed in any form).
 *  - "All Guests" tab is Admin-only — access code: "pokemon"
 *    The tab shows a password entry; on correct code it reveals the full list
 *    WITH the internal auto-generated ID visible for each record.
 *  - Regular tabs (Register / Update / Delete / Search) never show the ID.
 */
...
typedef struct {
    ...
    GtkWidget *lbl_next_id,*btn_save,*lbl_status;
    /* Update */
    ...
    /* Search (public list — no ID) */
    GtkWidget *entry_search,*tree,*lbl_count;
    GtkListStore *store;
    /* Admin list (with ID) */
    GtkWidget *admin_stack;        /* GtkStack: "lock" page vs "list" page */
...
static void refresh_list(AppWidgets *w,const char *filter){
    ...
    char idbuf[32];snprintf(idbuf,sizeof(idbuf),"Auto ID: %d",csv_next_id());
...
#define AR(l,ww) gtk_grid_attach(GTK_GRID(grid),gtk_label_new(l),0,row,1,1);\
    ...
    /* ID shown as read-only auto label */
    w->lbl_next_id=gtk_label_new("—");
    gtk_widget_set_halign(w->lbl_next_id,GTK_ALIGN_START);
    ...
    AR("Name",w->entry_name) AR("Age",w->entry_age) AR("Social Class",w->entry_class)
...
/* Admin-only tab — shows ID, protected by access code "pokemon" */
static GtkWidget *build_admin_list_tab(AppWidgets *w){
    ...
    gtk_entry_set_visibility(GTK_ENTRY(w->entry_admin_code),FALSE); /* show stars */
    ...
    /* Top bar: count label + lock button */
