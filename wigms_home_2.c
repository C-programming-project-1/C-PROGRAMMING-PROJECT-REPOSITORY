 ...
 *
 * Changes v3:
 *  - Dashboard tab is Admin-only — access code: "pokemon 1"
 *    A lock screen with a password entry guards the tab.
 *    Once unlocked, a Lock button returns to the lock screen.
 *  - Home tab unchanged (public).
 */
...
static void launch_app(const char *exe,GtkWidget *lbl){
    char cmd[512];
#ifdef _WIN32
...
static void on_btn_launch(GtkButton *btn,gpointer data){(void)btn;LaunchData *ld=(LaunchData*)data;launch_app(ld->exe,ld->status_lbl);}

/* ── Build home card ─────────────────────────── */
static GtkWidget *build_card(const char *title,const char *subtitle,const char *btn_lbl,const char *exe,GtkWidget *sl){
    ...
    LaunchData *ld=g_new0(LaunchData,1);ld->exe=exe;ld->status_lbl=sl;
    g_signal_connect(btn,"clicked",G_CALLBACK(on_btn_launch),ld);
...
static void activate(GtkApplication *app,gpointer user_data){
    ...
    gtk_window_set_default_size(GTK_WINDOW(window),800,540);
    ...
    /* Dashboard tab — locked by default */
    ...
    /* Footer */
    gtk_box_append(GTK_BOX(root),gtk_separator_new(GTK_ORIENTATION_HORIZONTAL));
    GtkWidget *footer=gtk_label_new("WIGMS v3.0  —  All .exe files must be in the same folder");
